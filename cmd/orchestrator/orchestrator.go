...
func NewCommand(ctx context.Context, version, commit string, logger *zap.Logger) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			tsdb, err := timescale.NewClient(ctx, cfg.DB.DSN, cfg.DB.ReplicaDSN)
			if err != nil {
	...
	cmd.AddCommand(newRecoveryCommand(ctx, logger, version, commit))
	cmd.AddCommand(newBackupCommand(ctx, logger, version, commit))
	return cmd
}
...
func newPreflightCommand(logger *zap.Logger) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Connect to database
			db, err := timescale.NewClient(cmd.Context(), cfg.DB.DSN, cfg.DB.ReplicaDSN)
			if err != nil {
