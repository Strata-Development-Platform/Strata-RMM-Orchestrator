...
# Preserve upstream 3.1.3 behavior while overriding only dependencies with
# fixes required by the current HIGH-severity vulnerability gate. The source
# tree is disposable, so tidy the patched module before compiling to ensure
# go.sum covers the complete transitive graph selected by these overrides.
go mod edit \
...
CGO_ENABLED=0 go build -trimpath -ldflags="-s -w" -o "$OLDPWD/$OUT_PATH" ./cmd/cosign

"$OLDPWD/$OUT_PATH" version >&2
printf '%s\n' "$COSIGN_VERSION source=$COSIGN_COMMIT x/text=$X_TEXT_VERSION grpc=$GRPC_VERSION" > "$OLDPWD/$OUT_DIR/BUILD-PROVENANCE.txt"
