...
const SingletonPlatformID = "00000000-0000-0000-0000-000000000001"
...
// business identity and its immutable setup-completion metadata.
...
// ProviderBusinessProfileValues is the complete set of editable values. API
// patch handlers merge partial input into a complete value before persistence.
...
// GetProviderBusinessProfile reads the singleton provider profile.
...
// LockProviderBusinessProfile reads and locks the singleton row for a merge
// followed by an update in the same transaction.
...
func getProviderBusinessProfile(ctx context.Context, db providerProfileQueryer, lock bool) (ProviderBusinessProfile, error) {
	query := `
		SELECT id::text, slug, legal_name, display_name, contact_name,
		       ...
		       tax_identifier, setup_completed_at,
	...
	err := db.QueryRowContext(ctx, query, SingletonPlatformID).Scan(
		...
		&completedAt, &profile.SetupCompletedBy, &profile.UpdatedAt,
	)
	if err != nil {
...
// UserCanManageProvider verifies an active owner/admin membership on the exact
// singleton platform scope. Callers still enforce token purpose and empty
// MSP/client/site context in the API layer.
...
// CompleteProviderSetup locks the singleton row, atomically writes the profile
// and audit event, and treats an identical retry as a successful no-op.
func CompleteProviderSetup(
	ctx context.Context,
	tx *sql.Tx,
	actorUserID string,
	values ProviderBusinessProfileValues,
) (ProviderBusinessProfile, bool, error) {
...
// UpdateProviderBusinessProfile changes only editable business-profile columns.
// Setup completion metadata and all unrelated platform state are never part of
// the UPDATE statement.
func UpdateProviderBusinessProfile(
	ctx context.Context,
	tx *sql.Tx,
	actorUserID string,
	values ProviderBusinessProfileValues,
...
func providerProfileUpdateSQL(completing bool) string {
	completion := ""
	if completing {
		completion = ", setup_completed_at = NOW(), setup_completed_by = $18::uuid"
	}
	return `
...
func providerProfileUpdateArgs(values ProviderBusinessProfileValues, actorUserID string, completing bool) []interface{} {
	args := []interface{}{
		...
		values.DefaultCurrency, values.TaxIdentifier,
	}
	if completing {
...
func changedProviderFields(before, after ProviderBusinessProfileValues) []string {
	...
	for i := 0; i < beforeValue.NumField(); i++ {
		if beforeValue.Field(i).String() != afterValue.Field(i).String() {
