...
| ID | Implementation/runtime consumer | Negative proof | Integration proof | Status |
|---|---|---|---|---|
| BDR-01 | Independent file key provider; `recovery key-init`; runtime resolution by key ID | missing/wrong key, duplicate initialization, permissions | rotated-key tests | verified |
| BDR-02 | Filesystem and S3 artifact repositories consumed by `backup.Engine` | malformed manifest, digest mismatch, path traversal | MinIO-backed S3 repository round trip | verified |
| BDR-03 | `PostgreSQLRecovery` runs credential-safe `pg_dump`/`pg_restore` against distinct databases | same target, missing target, corrupted artifact, command failure redaction | seeded source to clean target; schema, tenants, devices, jobs, audit | verified |
| BDR-04 | `JetStreamRecovery` captures streams, consumers, progress, headers, subjects, bytes | JetStream-disabled server, divergent non-empty target | real NATS delete/restore/verify/idempotent replay | verified |
| BDR-05 | `ObjectStorageRecovery` archives exact bytes, content type, metadata, per-object digests | corrupt restored object, same-target rejection in CLI factory | real MinIO source/target restore and idempotent replay | verified |
| BDR-06 | `backup.Engine` encrypts, persists, finalizes, verifies, decrypts, restores, and verifies all required components | tamper before mutation, missing component, failure cleanup | engine round trip plus service-backed component tests | verified |
| BDR-07 | Migration 65 creates shared recovery mutation gate and database triggers | non-owner resume; write while quiesced | real PostgreSQL gate test | verified |
| BDR-08 | Pinned session-level recovery lock | second operation times out; bounded release | real PostgreSQL concurrent lock test | verified |
| BDR-09 | API and dispatcher acquire shared recovery locks; DB trigger covers other writers | API returns 503; direct SQL mutation fails | PostgreSQL quiesce test | verified |
| BDR-10 | Restore verifies all artifacts before lock/quiesce/target mutation | ciphertext and manifest-associated-data tampering | engine tamper test | verified |
| BDR-11 | CLI backup/preflight/status/verify/restore/key-init execute real runtime factories | missing confirmation/target/config; no `--force` | CLI build and command tests | verified |
| BDR-12 | Source-independent verification and isolated restore target | unavailable source is not constructed for verify/restore | repository/key-only verification tests | verified |
| BDR-13 | Authenticated encryption only; manifest identity is associated data | nonce, ciphertext, key, metadata, truncation tampering | adversarial encryption suite | verified |
| BDR-14 | Exact-head CI and truthful evidence | stale/missing/non-terminal run rejected | Phase 8C and repository-wide workflows | implementation head passed 50/50; evidence-head run required |
...
| high | process arguments/errors could expose DSNs | credentials use libpq environment variables; only the database name is passed to `pg_restore`; redaction tests |
...
| real MinIO integration | pass in CI | Phase 8C run 30602575852 |
| real PostgreSQL integration | pass in CI | Phase 8C run 30602575852 |
...
| Implementation head | `4e2a3bc00172adb7363b880c03077b61589eafcd` |
| Phase 8C workflow | [run 30602575852](https://github.com/Strata-Development-Platform/Strata-RMM-Orchestrator/actions/runs/30602575852), 20/20 passed |
| Repository CI | [run 30602575803](https://github.com/Strata-Development-Platform/Strata-RMM-Orchestrator/actions/runs/30602575803), 30/30 passed |
| Final evidence head | recorded in PR body after its exact-head workflows complete |
