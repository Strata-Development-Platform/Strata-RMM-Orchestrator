...
| A8-06…A8-08 / Phase 8C | Pending exact-head run for PR #9 | Pending | Pending | Coordinator | CI pending |
...
| ID | Acceptance criterion | Current status |
|---|---|---|
| 8C-C01 | Independent key provider and external repository survive source loss | Implemented; exact-head CI pending |
| 8C-C02 | PostgreSQL backup restores schema, tenant data, durable jobs, and audit evidence into a distinct target | Real integration test added; exact-head CI pending |
| 8C-C03 | JetStream streams, consumer progress, headers, subjects, and messages restore into a distinct target | Real integration test added; exact-head CI pending |
| 8C-C04 | Object bytes, content type, metadata, and digests restore into a distinct target | Real integration test added; exact-head CI pending |
| 8C-C05 | Backup quiescing blocks database mutations and concurrent recovery | Migration 65 and PostgreSQL integration tests added; exact-head CI pending |
| 8C-C06 | Restore verifies every artifact before target mutation and always performs bounded cleanup | Engine failure/tamper tests pass locally; exact-head CI pending |
| 8C-C07 | CLI uses the real runtime path and has no validation bypass | Implemented; exact-head CI pending |
| 8C-C08 | Recovery documentation matches runtime behavior and limitations | Updated; review pending |
