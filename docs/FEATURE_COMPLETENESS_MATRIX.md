...
| Alert evaluation and state transitions | Verified core lifecycle; Partial for full specification | `internal/alerting/engine.go`, `rules.go`, `store.go` | `internal/alerting/engine_test.go` covers threshold and heartbeat fire/recovery correlation, cooldown, disabled rules, UUID persistence, restart restoration, and tenant/device-scoped CVE deduplication and resolution; PR #28 exact-head workflows passed 80/80; PR #81 adds 21 grouping tests + 7 API endpoints. | Grouping (severity, device, cascade, time-window) and maintenance-window silence **verified** (PR #81, 8 API endpoints, 21 tests). |
...
| Policy engine | Partial; lifecycle + enforcement verified | `internal/platform/policy_handlers.go`, `policy_model.go`, `policy_effective.go`, `policy_diff.go`, `policy_enforcement.go`, `policy_scheduler.go` | Model tests cover validation and recursive most-specific-wins merging; migration contracts cover immutable revisions; enforcement tests cover MSP/client/site/device scope discovery; **PR #100 added**: 68 behavioral tests covering policy categories and scope ranking, policy input normalization, policy validation (name/description/category/scope/child-scopes/config-depth/config-size), isValidTime/isValidDay, maintenance window validation, policy record/response fields, mergePolicyLayers/mergeMaintenanceLayers, computePolicyDiff, EffectivePolicy/PolicyEnforcementEngine/PolicyScheduler/PolicyAssignment. CI: all checks pass except pre-existing Durable Jobs Integration. | Authorized validation, preview, publication, revision history, and effective configuration are implemented. Automatic endpoint enforcement and scheduled re-evaluation added in PR #50. Script Vault and policy-to-script binding remain absent. |
| Scripts and durable endpoint jobs | Verified — 52 behavioral tests (PR #102) | `internal/agent/scripts/executor.go`, `internal/agent/jobs`, `internal/platform/script_schedule_runner.go`, `internal/platform/jobs.go`, `script_handlers.go`, `script_schedule_handlers.go` | agent jobs tests, jobs integration tests, endpoint approvals/audit tests, script_schedule_runner tests (dbintegration), **52 new behavioral tests** (PR #102) | Script execution dispatch via NATS verified. ScheduleOrchestrator.ExecuteSchedule dispatches to devices. ScriptScheduleRunner engine discovers due schedules and triggers execution. Agent echoes schedule_id/device_id in script_result for proper routing. Recurring schedules (hourly/daily/weekly/monthly) auto-reschedule after completion; one-shot "now" schedules transition to completed. Script result handler routes to ScheduleOrchestrator.ProcessScheduleDeviceResult when schedule_id present. **PR #102 adds 52 structural behavioral tests: ScriptEngine/Script/ScriptExecution struct field validation, Schedule/ScriptScheduleDeviceExecution struct field validation, language validation, schedule_type validation, timeout/retries bounds (max 720, min 15), default values, required field detection, schedule params defaults (weekly_days/weekly_hour/daily_hour/monthly_day), calculateNextRun deterministic results, joinUpdates separator behavior.** |
...
| Software deployment | Partial — 17 behavioral tests (PR #110) + 25 unit tests | `internal/agent/software/installer.go` | endpoint-operation tests, **25 new unit tests** (installer: MSI/EXE/DEB/RPM/AppImage/Script execution, checksum verification, timeout, cancelled context; handlers: engine creation, result structure). **PR #110 adds**: 17 structural behavioral tests covering SoftwareCommand struct fields (Type, DeploymentID, Action, SourceURL, Checksum, InstallArgs, UninstallArgs, PackageType, Timeout), SoftwareResult struct fields (Type, DeploymentID, Action, Status, ErrorMessage, DurationMs), Installer struct fields (nc, logger, tenantID, agentID, sub, httpClient), PackageType constants (msi, exe, deb, rpm, appimage, script), ActionType constants (install, uninstall), SoftwareCommand JSON round-trip, SoftwareResult JSON round-trip (with and without error messages), zero/empty field handling, timeout bounds validation, invalid package type handling, unknown action handling, Installer Start with nil NATS, Installer Stop idempotency, zero-value JSON round-trips. CI: all checks pass. | **MSI, EXE, DEB, RPM, AppImage, Script support added (PR #89)**: Complete package matrix with 6 package types, SHA256 verification, multi-device deployment lifecycle, timeout handling, context cancellation. 25 new unit tests. Upload distribution and uninstall behavior partially implemented. Representative OS execution tests limited by CI environment. |
| Third-party patch catalog | Partial | `internal/inventory/thirdparty.go`, `internal/platform/api.go`, `internal/platform/middleware.go` | `thirdparty_test.go`, `thirdparty_e2e_test.go` (27 new tests), `thirdparty_handlers_test.go` (22 new tests); CI: 83 pass (PR #91) | Live vendor discovery (`GET /api/v1/thirdparty/vendors`, `POST /api/v1/thirdparty/vendors/{vendor}/sync`, `GET /api/v1/thirdparty/vendors/status`), 24-hour sync scheduler, HTTP error handling in fetchLatestVersion, catalog vendor aggregation. **PR #91 added**: 49 unit tests covering version discovery (Chrome/GitHub/Zoom/7-Zip/LibreOffice/Mozilla mock servers), HTTP error handling (404, timeout, empty body), handler validation (all 7 endpoints), nil-engine error paths, deployment lifecycle states, concurrent access safety. nil DB guards added to `SyncAll`/`GetPackages`/`SyncApp`/`SyncVendor`. Vendor-specific version discovery and deployment workflow tests are verified; real OS deployment execution remains not accepted. |
| Remote support and recordings | Partial — 59 behavioral tests (PR #103) | `internal/platform/remote_handlers.go`, `internal/platform/api.go`, `internal/remote/capture.go`, `internal/remote/input.go`, `internal/remote/tunnel.go`, `internal/webrtc/config.go`, `internal/webrtc/handler.go`, `internal/webrtc/peer.go`, `internal/webrtc/recording.go`, `internal/webrtc/transcription.go` | `remote_session_binding_test.go`, recorder and recording-store tests, WebRTC: 80+ unit tests, LAN Cache: 45 unit tests, **59 new behavioral tests** (PR #103) | Interactive tunnel/recording routes registered (POST/DELETE sessions, recording management). Real Windows (BitBlt) capture, macOS (screencapture) capture, Linux (scrot/import/gnome-screenshot) capture implemented. Real Windows (SendInput) input injection, macOS (CGEvent) input injection, Linux (xdotool) input injection implemented. Interactive session endpoints active, recording start/stop/playback/list registered. **WebRTC Remote Support added (PR #88)**: P2P video sessions with TURN/STUN relay, session recording (WebM/MKV), live transcription (OpenAI Whisper, Azure Speech, Google Speech), 15 REST API endpoints, 80+ unit tests. **LAN Cache added (PR #88)**: local package distribution (Chocolatey, Winget, Flatpak, Snap, MSI, DEB, RPM), cache management, 5 REST endpoints, 45 unit tests. **PR #103 adds 59 structural behavioral tests: RemoteSessionRequest struct field validation, remoteSessionBinding struct fields and expiry behaviour, Frame struct fields, CaptureConfig defaults and bounds, InputEvent field validation, InputType/ModifierKeys constant validation, JSON round-trips for all types.** Representative-host acceptance remains absent. |
| Network discovery and flow monitoring | Partial — 87 behavioral tests (PR #104) | `internal/probe` | 22+ tests cover: ARP parsing (Linux/Windows), ping scan, port scanning (TCP SYN/UDP), service detection, flow parsing (NetFlow v5/v9/IPFIX), device type detection, MAC normalization, broadcast IP calculation, gateway discovery. All tests pass. **PR #104 adds**: 87 new behavioral tests covering DiscoveryResult/FlowRecord/PortScanResult struct fields, ServiceInfo/HostInfo/TopologyInfo/InterfaceInfo/AddressInfo/GatewayInfo struct fields and JSON round-trips, Config struct fields, SNMPV3Config/SNMPTarget struct fields, normalizeMAC/normalizeWindowsMAC/getBroadcastIP/protocolName utility function behavior. | `discoverARP` and `pingScan` return no results in isolated CI environments (expected — requires active network). LLDP/CDP/STP stub implementations added to discovery.go. Full topology discovery remains not implemented. |
| Vulnerability management | Partial — 91 behavioral tests (PR #105) | `internal/inventory/cve_sync.go`, `vulnerability.go`, `remediation.go`, `vulnerability_mgmt_test.go`, `version_helper_test.go` | `cve_sync_test.go`, **5 new vulnerability_mgmt tests** (version matching, lifecycle, detection), tenant/database suites, alert-engine tests, remediation engine wired with `patch.Executor` integration, **91 new behavioral tests** (PR #105) | OSV sync implemented. Version matching logic verified (5 tests: less than, equal, greater than, lifecycle transitions, detection accuracy). Package-version matching accuracy tested. Auto-remediation partially tested. Full manual lifecycle (open/patched/ignored) tested. Representative OSV/NVD sync requires external API access. **PR #105 adds 91 structural behavioral tests: CVERecord struct fields and JSON round-trip, VulnerabilityState struct fields and status value validation, VulnerabilityGroup struct fields, OSVSeverity/OSVPackage/OSVAffected/OSVRange/OSVReference structs fields and JSON round-trips, OSVVuln/OSVResponse struct fields, OSVQuery/SyncState struct fields, RemediationAttempt struct fields, RemediationPolicy struct fields, DeviceVulnerability struct fields, ComplianceReport struct fields, ComplianceFinding struct fields, versionLessThan utility function with 4 semantic version comparison tests.** External API sync (OSV/NVD) requires live network access. |
...
| Authentication and account lifecycle | Partial | `internal/platform/auth_handlers.go`, owner invitations, `pkg/auth/mfa.go` | auth, TOTP/MFA, invitation, provisioning, and isolation tests, **PR #98 added**: 76 behavioral tests covering JWT token generation/validation, authorization result behavior, TOTP provisioning, API key auth, security headers, invitation fields, login request validation, logout/me handler behavior. CI: all checks pass except pre-existing Durable Jobs Integration. | Password recovery, refresh-token rotation, and asymmetric access-token signing are absent or deferred. SSO/OIDC is explicitly future work. Trusted-proxy handling is deferred. |
...
| 8 | Script Vault & Policy Binding + Smart Group UI | Smart group script bindings (Migration 95, 3 endpoints, schedule dispatch), Smart Group UI components (ExpressionBuilder, DetailPage, Types, 14 tests), DeviceGroupListPage with real API integration, SmartGroupDetailPage with real API integration | **Complete** — 8A (PR #76, 66 pass), 8B (PR #77, 66 pass), 8C (PR #78, 101 frontend tests pass), 8D (PR #82, 110 frontend tests pass, 12 API methods, DeviceGroupListPage, SmartGroupDetailPage real API) | `internal/platform/smart_group_handlers.go`, `script_schedule_runner.go`, `jobs.go`, `ui/src/components/smart-groups/`, `ui/src/api/client.ts` |
