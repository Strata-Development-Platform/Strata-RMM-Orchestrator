...
| OS patch management | Partial; scan parsing wired | `internal/patch/executor.go`, `internal/patch/manager.go` | Patch scan parsing wired in executor.go: Windows PowerShell JSON → `[]*Patch` with Title/KB/Severity; apt list --upgradable → parsed package names and versions; dnf/yum check-update → parsed records; zypper list-patches → parsed records. Linux install uses `CombinedOutput` for real stdout/stderr capture. No automated patch executor tests yet. | Chocolatey, Wingnet, WSUS, Flatpak, Snap, canary, and rollback claims are not established. Patch scanning has no unit tests. Auto-remediation engine is wired but not tested end-to-end. |
...
| Network discovery and flow monitoring | Partial | `internal/probe` | No discovery or flow parser tests were found. | Flow parsing implements NetFlow v5/v9 and IPFIX parsers that correctly extract src/dst IPs, ports, protocol, bytes, packets, and duration. `discoverARP` and `pingScan` return no results. LLDP/CDP/STP topology and the full synthetic matrix are not implemented. |
