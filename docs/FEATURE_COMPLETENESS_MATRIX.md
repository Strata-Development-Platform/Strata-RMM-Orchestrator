...
| Software deployment | Partial | `internal/agent/software/installer.go`, `internal/platform/software_handlers.go`, `installer_test.go`, `software_handlers_test.go` | endpoint-operation tests, **25 new unit tests** (installer: MSI/EXE/DEB/RPM/AppImage/Script execution, checksum verification, timeout, cancelled context; handlers: engine creation, result structure) | **MSI, EXE, DEB, RPM, AppImage, Script support added (PR #89)**: Complete package matrix with 6 package types, SHA256 verification, multi-device deployment lifecycle, timeout handling, context cancellation. 25 new unit tests. Upload distribution and uninstall behavior partially implemented. Representative OS execution tests limited by CI environment. |
