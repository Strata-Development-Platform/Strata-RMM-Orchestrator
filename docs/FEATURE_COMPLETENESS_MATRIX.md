...
| 8 | Script Vault & Policy Binding + Smart Group UI | Smart group script bindings (Migration 95, 3 endpoints, schedule dispatch), Smart Group UI components (ExpressionBuilder, DetailPage, Types, 14 tests), DeviceGroupListPage with real API integration, SmartGroupDetailPage with real API integration | **Complete** — 8A (PR #76, 66 pass), 8B (PR #77, 66 pass), 8C (PR #78, 101 frontend tests pass), 8D (PR #82, 110 frontend tests pass, 12 API methods, DeviceGroupListPage, SmartGroupDetailPage real API) | `internal/platform/smart_group_handlers.go`, `script_schedule_runner.go`, `jobs.go`, `ui/src/components/smart-groups/`, `ui/src/api/client.ts` || 9 | OS Patch Management | Chocolatey, Winget, Flatpak, Snap package manager support; canary deployment validation with configurable pass threshold; patch rollback (Windows WSUS, Linux apt/dnf/yum/zypper); PatchPolicy with approval modes, severity filtering, maintenance windows | **Complete** — 9.1 (PR #83, 43 patch tests pass, all 29 packages ok, 110 frontend tests pass) | `internal/patch/executor_extended.go` |
