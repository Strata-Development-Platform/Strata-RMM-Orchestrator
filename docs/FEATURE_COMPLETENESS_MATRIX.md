...
| Alert evaluation and state transitions | Partial; implementation under exact-head verification | `internal/alerting/engine.go`, `rules.go`, `store.go` | `internal/alerting/engine_test.go` covers threshold and heartbeat fire/recovery correlation, cooldown, disabled rules, UUID persistence, and tenant/device-scoped CVE deduplication and resolution. | Grouping and maintenance-window silence remain absent; representative live notification delivery remains environment-dependent. |
| Alert notification delivery | Implemented; pending exact-head verification | `internal/alerting/notifier.go`, orchestrator configuration wiring | `internal/alerting/notifier_test.go`, configuration tests | Representative live-provider delivery remains environment-dependent; CI validates channel selection, fail-closed configuration, payload dispatch, and HTTP rejection behavior. |
| Policy engine | Partial | `internal/platform/policy_handlers.go`, policy schema | Route authorization/contract coverage only | Create/list/get/publish/delete exists, but validation, preview, revision history, effective inheritance, automatic enforcement, and scheduled re-evaluation are not implemented or behaviorally tested. |
...
| OS patch management | Partial | `internal/patch` | No patch executor tests were found. | Windows and Linux scans discard command output and return empty patch lists. Linux passes all selected packages as one command argument. Chocolatey, Winget, WSUS, Flatpak, Snap, canary, and rollback claims are not established. |
...
| Network discovery and flow monitoring | Partial | `internal/probe` | No discovery or flow parser tests were found. | `discoverARP` and `pingScan` return no results. Flow parsing only identifies a version and emits empty addresses, ports, byte, packet, and duration fields. LLDP/CDP/STP topology and the full synthetic matrix are not implemented. |
| Vulnerability management | Partial | `internal/inventory/cve_sync.go`, `vulnerability.go` | `cve_sync_test.go`; tenant/database suites cover storage boundaries; alert-engine tests cover scoped CVE alert correlation | Representative OSV/NVD synchronization, package-version matching accuracy, auto-remediation, and the full manual lifecycle still need end-to-end evidence. |
