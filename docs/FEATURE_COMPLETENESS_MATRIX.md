...
| Vulnerability management | Partial | `internal/inventory/cve_sync.go`, `vulnerability.go`, `remediation.go`, `vulnerability_mgmt_test.go` | `cve_sync_test.go`, **5 new vulnerability_mgmt tests** (version matching, lifecycle, detection), tenant/database suites, alert-engine tests, remediation engine wired with `patch.Executor` integration | OSV sync implemented. Version matching logic verified (5 tests: less than, equal, greater than, lifecycle transitions, detection accuracy). Package-version matching accuracy tested. Auto-remediation partially tested. Full manual lifecycle (open/patched/ignored) tested. Representative OSV/NVD sync requires external API access. |
