...
func TestCrossMSPAccessDenied(t *testing.T) {
	...
	tokenA, _ := gen.GenerateUserToken("tenant-a", "msp-a", "client-a", "", []string{"admin"}, time.Hour)
...
func TestCrossClientAccessDenied(t *testing.T) {
	...
	gen := auth.NewTokenGenerator("test-secret-that-is-long-enough-for-testing")
	token, _ := gen.GenerateUserToken("t1", "msp1", "client-a", "", []string{"admin"}, time.Hour)
...
func TestOwnMSPSucceeds(t *testing.T) {
	...
	gen := auth.NewTokenGenerator("test-secret-that-is-long-enough-for-testing")
	token, _ := gen.GenerateUserToken("t1", "msp-a", "", "", []string{"admin"}, time.Hour)
...
func TestSuspendedMSPScenario(t *testing.T) {
	...
	gen := auth.NewTokenGenerator("test-secret-that-is-long-enough-for-testing")
	token, _ := gen.GenerateUserToken("t1", "suspended-msp", "", "", []string{"admin"}, time.Hour)
