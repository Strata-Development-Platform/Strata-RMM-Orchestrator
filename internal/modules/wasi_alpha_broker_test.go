...
func TestSignedWASIReferenceCallsDevicesGetThroughBrokerABI(t *testing.T) {
	...
	if _, err := MaterializePayloadRetrySafe(pkg, payload, MaterializeOptions{Root: root}); err != nil { t.Fatal(err) }
	if _, err := ActivateMaterializedVersion(root, manifest.ID, manifest.Version); err != nil { t.Fatal(err) }
	runtime, err := NewWASIRuntime(WASIRuntimeOptions{Root: root, Broker: broker})
	if err != nil { t.Fatal(err) }

	result, err := runtime.Invoke(context.Background(), enabled, Invocation{Method: "GET", Path: "/api/modules/alpha.reference/device", Permission: "devices.read", Scope: scope})
	if err != nil { t.Fatalf("brokered WASI invocation: %v", err) }
	if result.StatusCode != 200 { t.Fatalf("status = %d, want 200", result.StatusCode) }
	if resolver.seen != deviceID { t.Fatalf("resolver saw device %q, want %q", resolver.seen, deviceID) }
}
...
func TestSignedWASIReferenceBrokerDeniesCrossScopeDevice(t *testing.T) {
	...
	runtime, err := NewWASIRuntime(WASIRuntimeOptions{Root: root, Broker: broker})
	if err != nil { t.Fatal(err) }
	if _, err := runtime.Invoke(context.Background(), enabled, Invocation{Method: "GET", Path: "/api/modules/alpha.reference/device", Permission: "devices.read", Scope: wrongScope}); err == nil {
		t.Fatal("cross-scope broker access unexpectedly succeeded")
	}
	if resolver.seen != deviceID { t.Fatalf("resolver did not authoritatively resolve requested device; saw %q", resolver.seen) }
}

type alphaBrokerResolver struct { device BrokerDevice; seen string }
func (r *alphaBrokerResolver) ResolveBrokerDevice(_ context.Context, id string) (BrokerDevice, error) {
	r.seen = id
	if id != r.device.ID { return BrokerDevice{}, fmt.Errorf("unknown device") }
	return r.device, nil
}
...
func alphaDevicesGetWASM(deviceID string) []byte {
	...
	body := []byte{0x00}
	for _, value := range []int32{0, int32(len(operation)), 64, int32(len(input)), 256, 512} {
		body = append(body, 0x41)
		body = append(body, alphaSLEB(value)...)
	}
...
func alphaActiveDataSegment(offset uint32, data []byte) []byte {
	...
	segment = append(segment, alphaSLEB(int32(offset))...)
...
func alphaWASMName(value string) []byte { encoded := alphaULEB(uint32(len(value))); return append(encoded, []byte(value)...) }
func alphaULEB(value uint32) []byte {
	var out []byte
	for { b := byte(value & 0x7f); value >>= 7; if value != 0 { b |= 0x80 }; out = append(out, b); if value == 0 { return out } }
}
func alphaSLEB(value int32) []byte {
	var out []byte
	for {
		b := byte(value & 0x7f)
		next := value >> 7
		done := (next == 0 && b&0x40 == 0) || (next == -1 && b&0x40 != 0)
		if !done { b |= 0x80 }
		out = append(out, b)
		value = next
		if done { return out }
